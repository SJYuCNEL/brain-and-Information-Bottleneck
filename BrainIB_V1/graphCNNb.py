...
class GraphCNN(nn.Module):
    def __init__(self, num_layers, num_mlp_layers, input_dim, hidden_dim, output_dim, final_dropout, learn_eps, graph_pooling_type, neighbor_pooling_type, device):
        ...
        self.linear1 = nn.Linear(self.total_latent_dim, output_dim)
    ...
    def dim_reduction(self, batch_graph, num_reduce):

        reduced_batch_graph=batch_graph

        #num_features=len(batch_graph[0].node_features[-1])
        
        #matrix = Variable(torch.rand(num_features,num_reduce))
        #matrix=matrix*0.5/num_reduce
        #kernel_P=F.relu(matrix)
        for g in reduced_batch_graph:
            adj_matrix=g.node_features
            rand_arr = np.arange(adj_matrix.shape[1])
            np.random.shuffle(rand_arr)
            g.node_features=g.node_features[:,rand_arr[0:5]]
            #AF=torch.tensordot(adj_matrix,kernel_P, dims=[[-1],[0]])
            #data=torch.tensordot(kernel_P,AF, dims=[[0],[0]])
            #data=data.transpose(1,0)
            #_range = np.max(data) - np.min(data)
            #data = (data - np.min(data)) / _range
            #g.node_features=torch.FloatTensor(data)

        return reduced_batch_graph
    ...
    def forward(self, batch_graph):
        ...
        #batch_graph= self.dim_reduction(batch_graph, self.num_reduce)
        ...
        for i in range(len(graph_sizes)):
            batch.append(torch.LongTensor([i]*116))

        batch = torch.cat(batch, 0).to(self.device)

        graph_emb = global_mean_pool(hidden_rep,batch)
        ...
        #     new_batch_graphs = torch.tanh(batch_graphs)

        score = F.dropout(self.linear1(graph_emb), self.final_dropout, training=self.training)

        return graph_emb ,score
